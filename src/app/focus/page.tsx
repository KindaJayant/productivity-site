...
export default function FocusMode() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("brutal_focus_sessions");
    if (stored) {
      try {
        ...
        console.error("Failed to parse stored focus sessions", e);
      }
    }
    setMounted(true);
  }, []);
  ...
  useEffect(() => {
    if (mounted) {
      localStorage.setItem("brutal_focus_sessions", JSON.stringify(sessions));
    }
  }, [sessions, mounted]);
  ...
  const handleCreateSession = () => {
    ...
    const newSession: Session = {
      id: uuidv4(),
      title: `Workflow ${sessions.length + 1}`,
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      console.error(error);
      setSessions(prev => prev.map(s => 
        s.id === activeSessionId ? { ...s, messages: [...newMessages, { role: "assistant", content: "Failed to connect to the Focus Engine." }] } : s
      ));
    } finally {
      setLoading(false);
    }
