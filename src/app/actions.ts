...
export async function submitFocus(input: string) {
  const messages: Message[] = [{ role: "user", content: input }];
  return callAgent(messages, FOCUS_PROMPT);
}
...
export async function submitRubberDuck(input: string) {
  const messages: Message[] = [{ role: "user", content: input }];
  return callAgent(messages, DUCK_PROMPT);
}
...
export async function submitAccountability(input: string, context?: string) {
  // We can pass context about their previous days directly into the user message
  const userMessage = context 
    ? `Recent Context:\n${context}\n\nToday's Log:\n${input}`
    : input;
    
  const messages: Message[] = [{ role: "user", content: userMessage }];
  return callAgent(messages, ACCOUNTABILITY_PROMPT);
}
